...
def CRPS_ML(y_pred, y_true, **kwargs):
    ...
    This implementation assumes that the predictions are normal distributions
    with parameters mu and sigma, where mu is the mean and sigma is the standard
    deviation. The predictions should be in the form of an array with shape
    (n_samples, 2*n_features), where the odd columns contain the mean (mu)
    and the even columns contain the standard deviation (sigma).
